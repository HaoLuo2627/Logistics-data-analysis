...
def suggestion(cities, weights):
    ...
    weightDistance = np.zeros((mFactorial * nFactorial, 1))
    routeList = []
    for k in range(mFactorial):
        for l in range(nFactorial):
            ...
            for j_ in range(len(trackList)-1):
                ...
                if j_ < len(sourceIndexList[k]):
                    weight += weights.loc[trackList[j_], :].sum()
                    weightDistance[k * nFactorial + l, :] += weight*distanceDict[key]
                elif j_ < len(trackList)-1:
                    weight -= weights.loc[:, trackList[j_]].sum()
                    weightDistance[k * nFactorial + l, :] += weight*distanceDict[key]
            route = parseRoute(trackList[0], route)
            routeList.append(route)
    # 找出距离最小的路线
    ind = np.argsort(weightDistance, axis=0)
    ...
    weightDistanceInTotal = weightDistance[ind,0][:3]
    ...
    return distanceInTotal,weightDistanceInTotal, suggestPath, suggestRoute
...
if __name__ == '__main__':
    ...
    cities = {'提货':['陕西,咸阳'], '卸货':['陕西,西安','青海,西宁',\
                                    '陕西,安康']}
    # 增加考虑运输量，让运输量与长度的乘积之和最小
    weights = pd.DataFrame([[20, 40, 60]],index=cities['提货'],columns=cities['卸货'])
    #==========================================================================
    # 模型参数
    alpha = 0.2
    beta = 0.3
    ...
    # 基于图论优化，找出经过这些点的最短路
    distanceInTotal, weightDistanceInTotal, suggestPath, suggestRoute = suggestion(cities, weights)
    ...
    if trackDistribution.shape[0] > 0:
        for idx in range(trackDistribution.shape[0]):
            ...
            for ddd in range(len(tmp) - 1):
                ...
                if tmp[ddd][0] == '提货':
                    weight += weights.loc[tmp[ddd][1], :].sum()
                elif tmp[ddd][0] == '卸货':
                    weight -= weights.loc[:, tmp[ddd][1]].sum()
                weightDis += weight * disTmp
            route = parseRoute(tmp[0][1], route)
            trackDistribution.loc[idx, 'distance'] = dis
            trackDistribution.loc[idx, 'route'] = route
            trackDistribution.loc[idx, 'weightDistance'] = weightDis
        print('历史上符合查询要求的运单的路线和选择这条路线的人数')
        print(trackDistribution[['track','freq','distance','weightDistance']])
    else:
        print('历史上没有符合要求的运单')
                     ...
                     'freq':0,\
                         # 'percentage':0,\
                             'hot':sigmoid(-0.5 * np.log(1 + trackCount['freq'].max())),\
                                 ...
                                 'weightDistance':weightDistanceInTotal[idx,0]},\
                    ignore_index=True)
    #计算路线长度度量
    distance = trackDistribution['distance']
    if trackDistribution.shape[0] > 1:
        trackDistribution['short'] = sigmoid(2 * (distance.mean() - distance) / distance.std())
    else:
        trackDistribution['short'] = 1
    #计算成本度量
    weightDistance = trackDistribution['weightDistance']
    if trackDistribution.shape[0] > 1:
        trackDistribution['cost'] = sigmoid(2 * (weightDistance.mean() - weightDistance) / weightDistance.std())
    else:
        trackDistribution['cost'] = 1
    # calculate score
    trackDistribution['score'] = alpha * trackDistribution['hot'] + \
        beta * trackDistribution['short'] + \
            gamma * trackDistribution['cost']
